...
std::vector<TargetInfo> processImpl(int w, int h, int texOut, DisplayMode mode,
  ...
  for (auto &contour : contours) {
    ...
    TargetInfo target;
    target.centroid_x = boundingRect.x + boundingRect.width / 2.0;
    target.centroid_y = boundingRect.y + boundingRect.height / 2.0;
    target.width = boundingRect.width;
    target.height = boundingRect.height;

    poly.push_back(boundingRect.tl());
    poly.push_back(cv::Point(boundingRect.br().x, boundingRect.tl().y));
    poly.push_back(boundingRect.br());
    poly.push_back(cv::Point(boundingRect.tl().x, boundingRect.br().y));
    target.points = poly;

    // Filter based on size
    // Keep in mind width/height are in imager terms...
    const double kMinTargetWidth = 10;
    const double kMaxTargetWidth = 100;
